...
- Pagina publica da rifa implementada e validada em ambiente Vercel + Neon
- Fluxo do participante publicado sem OTP, com geracao de uma cota por confirmacao
- Grid publica de cotistas removida da tela publica publicada
...
- Brevo validado com envio real sem intercorrencias registradas
...
- Reserva recorrente de cotas, uma por confirmacao
...
- gerar uma cota aleatoria de 5 digitos por confirmacao
...
Status: DONE FOR MVP ACCEPTANCE
...
- validacao funcional em Vercel + Neon confirmada pelo usuario
- envio real por Brevo confirmado pelo usuario
- ausencia de intercorrencias registrada pelo usuario

Backlog pos-plano:
- testes automatizados de unidade, integracao e E2E
- revisao formal de seguranca
- aprofundamento de hardening alem do MVP
...
- [x] Ajustar a pagina publica para coletar nome, email e telefone em uma confirmacao unica.
...
- validacao funcional em ambiente Vercel + Neon confirmada pelo usuario
- envio de email via Brevo confirmado pelo usuario
- ausencia de intercorrencias no fluxo validado confirmada pelo usuario

Pendente:
- nenhum item bloqueante para este plano

## Risks and Gaps

- Confirmacao de pagamento ainda e manual no MVP
- Ainda nao ha suite automatizada cobrindo sorteio e autorizacao do admin
- Hardening adicional ficou aceito como backlog fora deste plano

## Next Execution Steps

1. Encerrar formalmente este plano.
2. Iniciar novo plano para o follow-up de pagamento e os ajustes de copy solicitados.

## Completion Criteria

Este plano foi marcado como `DONE` porque:
...
- Ambiente remoto validado pelo usuario: Vercel + Neon
- Email transacional validado pelo usuario: Brevo
- Incidentes registrados no fluxo validado: nenhum
- Commits de entrega que consolidaram a versao aceita:
  - `c837908` — retirada de OTP e inclusao de email Brevo
  - `f22f939` — landing e fluxo de recibo por email
  - `171f1f5` — melhorias de edicao da rifa
  - `909fe46` — refinamento do fluxo publico e CTA de nova cota
  - `f0848cf` — retirada da grid publica de cotistas e refinamento do admin
